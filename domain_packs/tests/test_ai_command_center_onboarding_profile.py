...
def test_ai_command_center_onboarding_profile_is_outcome_led_and_non_authorizing() -> None:
    ...
    assert [group["source_group_id"] for group in profile["source_groups"] if group["default_selected"]] == [
        "official_records"
    ]
    assert profile["source_groups"][0]["access_label"] == ("Recorded demo pair available · other sources proposed")
    assert all(group["access_label"] == "Proposed · not connected in v1" for group in profile["source_groups"][1:-1])
